...
Usage:
    python phase_viewer.py           # decode fresh (shows progress)
    python phase_viewer.py --cached  # reload from saved video cache
...
CACHE_VID  = Path(__file__).parent.parent / "outputs" / "phase_viewer_cache.mp4"
...
def build_frames(ep_df, phases, episodes):
...
def save_cache(strips):
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    h, w   = strips[0].shape[:2]
    writer = cv2.VideoWriter(str(CACHE_VID), fourcc, 1.0, (w, h))
    for s in strips:
        writer.write(s)
    writer.release()
    print(f"Cache saved -> {CACHE_VID}")


def load_cache():
    cap = cv2.VideoCapture(str(CACHE_VID))
    strips = []
    while True:
        ret, frame = cap.read()
        if not ret:
...
def main():
    ...
    if args.cached and CACHE_VID.exists():
        ...
        strips = build_frames(ep_df, phases, episodes)
        save_cache(strips)

    print(f"Loaded {len(strips)} episodes. Opening viewer...")
    run_viewer(strips)
